...
- R-002: URLs containing userinfo (`user:pass@host`) MUST be rejected entirely [NFR-016].
...
1. **Param blocklist**: drop URLs whose query contains a parameter name matching [CFG-029] patterns → ST-190/`TRAP_PARAM`.
2. **Path budget**: per Host + normalized-path-shape (digits replaced by `N`), if shape count > [CFG-030] → ST-190/`TRAP_PATH_BUDGET`.
3. **Depth**: depth > [CFG-004] → ST-190/`DEPTH_LIMIT`.
