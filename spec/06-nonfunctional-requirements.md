...
| NFR-004 | Disk | Content Store growth is bounded: at most [CFG-005] distinct payloads per recrawl generation, with at most ceil([CFG-027] × 86400 / ([CFG-025] × (1 − [CFG-026]))) generations retained when recrawl is enabled ([CFG-025]>0; [CFG-027] is days, [CFG-025] seconds — the formula converts to seconds before dividing; the divisor is the *minimum* jittered interval: jitter only shrinks intervals and the 304 multiplier (≤ 4×) only grows them, so the bound holds for every URL; [CFG-026] < 1 by range, so the divisor is positive), else one; the retention job enforces [DOC-11 §6]. |
